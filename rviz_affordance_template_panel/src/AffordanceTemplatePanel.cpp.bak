#include "AffordanceTemplatePanel.hpp"
...
AffordanceTemplatePanel::AffordanceTemplatePanel(QWidget* parent)
    : rviz::Panel(parent),
      context(1),
      connected(false) {
    setupWidgets();
...
AffordanceTemplatePanel::~AffordanceTemplatePanel() {
    delete gridLayout;
...
void AffordanceTemplatePanel::setupWidgets() {
...
void AffordanceTemplatePanel::getAvailableTemplates() {
    if (connected) {
...
string AffordanceTemplatePanel::resolvePackagePath(const string& str) {
...
void AffordanceTemplatePanel::removeTemplates() {
...
void AffordanceTemplatePanel::sendAdd(const string& class_name) {
    Request req;
...
void AffordanceTemplatePanel::sendKill(const string& class_name, int id) {
    Request req;
...
void AffordanceTemplatePanel::killTemplate(QListWidgetItem* item) {
...
void AffordanceTemplatePanel::sendShutdown() {
    Request req;
...
void AffordanceTemplatePanel::sendPing() {
    Request req;
...
void AffordanceTemplatePanel::getRunningTemplates() {
    Request req;
...
void AffordanceTemplatePanel::connect() {
...
void AffordanceTemplatePanel::disconnect() {
...
void AffordanceTemplatePanel::addTemplate() {
...
void AffordanceTemplatePanel::send_request(const Request& request, Response& response) {
    if (connected) {
        try {
            string req;
...
bool AffordanceTemplatePanel::addAffordance(const AffordanceSharedPtr& obj) {
...
bool AffordanceTemplatePanel::removeAffordance(const AffordanceSharedPtr& obj) {
    // check if template is in our map
    if (checkAffordance(obj)) {
...
bool AffordanceTemplatePanel::checkAffordance(const AffordanceSharedPtr& obj) {
...
#include <pluginlib/class_list_macros.h>
    ...
    PLUGINLIB_EXPORT_CLASS(rviz_affordance_template_panel::AffordanceTemplatePanel, rviz::Panel)
#else
    // PLUGINLIB_DECLARE_CLASS(pkg, class_name, class_type, base_class_type)
    PLUGINLIB_DECLARE_CLASS(rviz_affordance_template_panel, AffordanceTemplatePanel, rviz_affordance_template_panel::AffordanceTemplatePanel, rviz::Panel)
#endif
