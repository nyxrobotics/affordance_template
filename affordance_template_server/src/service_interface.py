...
class ServiceInterface(object):
    def __init__(self, server):
        ...
        self.robot_info_service = rospy.Service('get_robot_config_info', GetRobotConfigInfo, self.handle_robot_request)
        # self.template_info_service = rospy.Service('get_affordance_template_info', GetAffordanceTemplateInfo, self.handle_template_request)
        
    def handle_robot_request(self,req) :
        print "requested robot info: ", req.name
        resp = GetRobotConfigInfoResponse()

        if req.name == "" :
            for r in self.server.robot_map.keys() :
                resp.robots.append(self.server.robot_map[r])
        else :
            if req.name in self.server.robot_map.keys() :
                resp.robots.append(self.server.robot_map[req.name])
        return resp;
